...
Because PCR methods work best with numerical data, we first dropped all factor variables from our dataset, keeping only integer and numerical datatypes
```{r dropfactors}
...
The dataset now contains `r dim(df_train_num)[2]` variables. We then generate a general pcr model using all the variables in this dataset to predict saleprice.
```{r modelGeneration}
