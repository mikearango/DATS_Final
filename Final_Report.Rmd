...
# output to table
stargazer(prediction_rf1_rf2_test, type="html", title="Table 5: Prediction Results", header=FALSE, summary.stat=c("n","median", "sd", "min", "max"), digits=2)
```

## Prediction for individual house

Using the following input paramters (from the 1st observation of the test data), the rf1 model predicts the sale price to be \$`r round( predict(modelrf1, test_x[1,]),2)`, and rf2 model predicts sale price to be \$`r round( predict(modelrf1, test_x[1,]),2)`. The true price of this house is $`r round(test_y[1,], 2)`.
...
## Example For Changing One Feature - Garage

If we change the garage size from a `r test_x[1,'garage.cars']` car garage to a 3 car garage, model rf1 predicts price to increase to \$`r round(predict(modelrf1, update_test_x_1),2)` (a `r round( (predict(modelrf1, update_test_x_1) - predict(modelrf1, test_x[1,]) )/predict(modelrf1, test_x[1,]),4)*100`% change in sale price). Model rf2 predicts price to increase to \$`r round(predict(modelrf2, update_test_x_1),2)` (a `r round( (predict(modelrf2, update_test_x_1) - predict(modelrf2, test_x[1,]) )/predict(modelrf2, test_x[1,]),4)*100`% change in sale price).
