...
From the barplot of `bsmt.exposure` it seems as if there is an unnamed level of the variable. Looking at median sale price by the type of basement exposure, we can see this is true. These 3 observations are dropped.
...
Sale Type (Nominal): Cleaning up trailing spaces in code.
...
We are combining all Non New TYpes into one and exclude COD rows

      WD, CWD, VWD Con, ConLw, ConLI, ConLD, Oth -> Existing
      New -> New
      COD -> drop observation   (see T Test results)
      
      Note that new will overlap with house.age.  We believe that the New attribute indicates no prior occupant regardless of the age of the house in much the same fashion as the treatment of new or used cars.  We have corrected the instance where age is 89 years and flagged as new
...
We recommend that for the purpose of forecasting a price:

1. Include and classify as Normal

       Normal	Normal Sale [No action]
      
       Partial	Home was not completed when last assessed (associated with New Homes)
        [It should be noted that Sale TYpe of "New" overlaps with Sales Condition of "Partial".  All but 6 of the Partial are also New Sales Type. We therefore recommend that we recode "Partial" as "Normal" and rely on the Sales TYpe of "New"]

       Alloca	Allocation - two linked properties with separate deeds, typically condo with a garage unit 
        [It should be noted that we have 16 properties whose sales are clearly linked i.e, same sale month/year, same neighbood, same leading PID -- We believe that the purchase price has been allocated between the two entries and have therefore recoded these as Normal]
        
       AdjLand	Adjoining Land Purchase
        [It should be noted that we have 9 properties whose sales are clearly linked i.e, same sale month/year,           same neighbood, same leading PID. -- We believe that the purchase price has been allocated between the two entries and have therefore recoded these as Normal]
        
2. Exclude uncertain terms and conditions.  The results of a T-test against purchase price indicate that a siginificant difference exists between the prices for sales with these conditions which warrants their removal from the price prediction dataset.

       Abnorml	Abnormal Sale -  trade, foreclosure, short sale -> 188

       Family	Sale between family members -> 46
...
```
As a result the transformation, all Sales are now treasted as Normal

## Creating a Saledate 

We are creating a sale date as a combination of the mo.sold, a constant "01" [Note: There is no day.sold] and yr.sold to help with a potential time series analysis.

```{r sale date creation}
...
I am waiting until all of our cleaning is done to uncomment this. 