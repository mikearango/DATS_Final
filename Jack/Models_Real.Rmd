...
The RMSE of reduced model is 30200.18, an increase from the full model. The model is furthur simplified to see if this trend continues as simplifcation increases.  
...
The RMSE continues to increase with each model simplication. A stepwise regression using backward elimination to run to find the model with the lowest AIC.   
...
To explore other model creation options, a simple decision tree regression is run. A decision tree will binarily split the data off a single predictor and create leafs, which will then be split off another predictor until    
...
The decision tree model produces worse results than the full model. 
