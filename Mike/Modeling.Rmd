...
Removing zero variance predictors as opposed to near-zero variance predictors before centering, scaling, and running PCA increased the RMSE slightly. 
...
Our grid search show that a lambda of 5000 minimizes the RMSE of our model. By finding the optimal lambda, we were able to decrease our RMSE again.  
...
As we can see from the code output, the forward selection model with no pre-processing does not turn out very well because of high levels of multicollinearity in the data. So, in the next forward selection model we pre-process heavily. 

```{r forward2}
set.seed(442)
# Train glmnet with custom trainControl and tuning: model
forward_model2 <- train(
  # formula
  saleprice ~ ., 
  # data
  training,
...
# Print model to console
print(forward_model2)
# plot RMSE vs alpha
plot(forward_model2)
# Print minimum RMSE
print(min(forward_model2$results$RMSE))
```

This drastically decreases the RMSE from the first forward selection model, but it is still high compared to our better models. 

### Mixed Steps

We decided to run a mixed step-wise regression model with the same pre-processing techniques as the second forward selection model. 
...
model_list <- list(rf1= modelrf1, rf2 = modelrf2, rf3 = modelrf3, rf4 = modelrf4, elastic1 = enet1, 
                   elastic2 = enet2, forward1 = forward_model, forward2 = forward_model2, mixed = mixed_model)
...
```{r test}
p <- predict(modelrf1, test)
error <- p - test$saleprice
RMSE <- sqrt(mean(error^2))
RMSE

p2 <- predict(modelrf2, test)
error2 <- p2 - test$saleprice
RMSE2 <- sqrt(mean(error2^2))
RMSE2

p3 <- predict(enet2, test)
error3 <- p3 - test$saleprice
RMSE3 <- sqrt(mean(error3^2))
RMSE3

p4 <- predict(enet1, test)
error4 <- p4 - test$saleprice
RMSE4 <- sqrt(mean(error4^2))
RMSE4
```

The test RMSE for the first random forest model ends up being the best at `r RMSE`. 
